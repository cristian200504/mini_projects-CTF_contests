# the-clock (LACTF 2026) Writeup

## Summary

This is a crypto challenge based on a "Clock Group" (a group defined on the circle $x^2 + y^2 = 1$). It implements a Diffie-Hellman-like key exchange. The challenge provides a base point, Alice's public key, Bob's public key, and an encrypted flag.

## Solution

The security of the key exchange relies on the Discrete Logarithm Problem (DLP) in the Clock Group.
1. The problem is to find Alice's secret $a$ such that $Base^a = AlicePub$.
2. The order of the group (number of points) is related to $p$. Specifically for this group, the order is likely $p+1$ (or a large factor of it).
3. The solver script recovers the prime $p$ from the given points (since $x^2+y^2-1 = 0 \pmod p$).
4. It determines the group order $N$.
5. Crucially, the group order $N$ (or the order of the base point) is **smooth**, meaning it is composed of many small prime factors.
6. This weakness allows the usage of the **Pohlig-Hellman algorithm** to solve the Discrete Logarithm Problem efficiently.
7. The solver factors the order, computes the discrete log modulo each small prime factor, and combines them using the Chinese Remainder Theorem (CRT) to recover the full secret $a$.
8. With $a$, the shared secret $BobPub^a$ is computed, the AES key is derived, and the flag is decrypted.
