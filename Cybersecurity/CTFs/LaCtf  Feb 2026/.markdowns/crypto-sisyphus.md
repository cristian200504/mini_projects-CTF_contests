# sisyphus (LACTF 2026) Writeup

## Summary

This is a crypto challenge utilizing AES-CTR. The server generates a table of encrypted values and wire labels, seemingly implementing a Garbled Circuit or similar protocol, but with reused keys/IVs.

## Solution

The vulnerability lies in the reuse of the IV and key stream in AES-CTR mode.
1. The server provides `wire 0` and `wire 1` values for two input wires, let's say `A` and `B`.
2. It also provides an encrypted table for the logic gate.
3. AES-CTR encryption is `Ciphertext = Plaintext ^ Stream(Key, IV)`.
4. We are given the keys for the input wires (`KeyA`, `KeyB`) and the IV.
5. The output keys (`L_out`) are encrypted using the stream generated from the input keys.

By XORing the provided table entries (Ciphertexts) with the streams generated from the known input keys (`StreamA`, `StreamB`), we can recover the output keys (`L_out_0`, `L_out_1`). The challenge is to predict the output label for a specific input case (Input A=1, B=1).

The solver script `solve.py` calculates the streams for the given keys, and then iteratively decrypts the table entries to recover the wire label for the "1" output (`L_out_1`), which is then sent to the server to claim the flag.
