# lazy-bigrams (LACTF 2026) Writeup

## Summary

The challenge provides a ciphertext `ct.txt` and a python script `chall.py`. The script encrypts a plaintext by mapping characters to their phonetic alphabet representation (A -> ALPHA, etc.), potentially adding an 'X' padding, and then performing the mapping again. The result is a substitution cipher where bigrams in the plaintext correspond to bigrams in the ciphertext.

## Solution

The solution involves reversing the double phonetic mapping. Since the mapping is deterministic and the search space is small (lowercase letters, numbers, `_`, `{`, `}`), we can recover the flag character by character.

The solver script (`solve.py`) implements a depth-first search (or greedy approach) to build the flag string `lactf{...}`. For each candidate character appended to the current flag prefix, it simulates the double phonetic mapping and checks if the resulting bigrams are consistent with the ciphertext bigrams (using a forward and reverse mapping consistency check).

By iteratively extending the flag and verifying consistency against the ciphertext bigrams, the script successfully recovers the original flag.
