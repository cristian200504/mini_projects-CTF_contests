"""
Extended Hidden Number Problem solver.
Adapted (import path only) from jvdsn/crypto-attacks (attacks/hnp/extended_hnp.py),
implementing Hlavac M., Rosa T., "Extended Hidden Number Problem and Its
Cryptanalytic Applications" (Section 4).
"""
...
from lattice_shared import closest_vectors
...
def attack(x_, N, pi, nu, a, p, u, b, delta=None):
    ...
    print(f"[extended_hnp] d={d} m={m} L={L} D={D} KD~={float(KD):.3e} delta~={float(delta):.3e}", flush=True)
